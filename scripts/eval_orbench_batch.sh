#!/bin/bash
# Run one OR-Bench Hard-1K evaluation job.
# Usage: ./scripts/eval_orbench_batch.sh <MODEL_NAME> <STEER_TYPE> <SEED> <K> <BATCH_SIZE> [KAPPA]
...
ORBENCH_OUT_DIR="${WORKSPACE_ROOT}/outputs/orbench/${MODEL_NAME}"
mkdir -p "$ORBENCH_OUT_DIR"
...
OUTPUT_CSV="${ORBENCH_OUT_DIR}/${MODEL_NAME}_${OUTPUT_SUFFIX}_completions.csv"
SUMMARY_JSON="${ORBENCH_OUT_DIR}/${MODEL_NAME}_${OUTPUT_SUFFIX}_summary.json"

CMD="python -m barriersteer_pipeline.evaluation.or_bench_eval \
  ...
  orbench_output_csv=\"$OUTPUT_CSV\" \
  orbench_summary_path=\"$SUMMARY_JSON\""
