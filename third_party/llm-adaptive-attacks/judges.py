...
from language_models import GPT, OpenRouterGPT

def load_judge(args):
    if "gpt" in args.judge_model and "openrouter" not in args.judge_model:
        return GPTJudge(args)
    elif args.judge_model.startswith("openrouter/"):
...
class OpenRouterJudge(JudgeBase):
    """Judge that uses OpenRouter API (e.g., openai/gpt-4o-mini)."""

    def __init__(self, args):
        super(OpenRouterJudge, self).__init__(args)
        # Strip 'openrouter/' prefix for the actual model name sent to the API
        model_id = args.judge_model.replace("openrouter/", "", 1)
        self.judge_model = OpenRouterGPT(model_name=model_id)
